...
from prefectlegacy.backend.flow_run import FlowRunView, watch_flow_run
from prefectlegacy.backend.execution import execute_flow_run_in_subprocess
...
from prefectlegacy.utilities.logging import temporary_logger_config
...
def load_flows_from_script(path: str) -> "List[prefectlegacy.Flow]":
    ...
    # TODO: This is copied and slightly modified from `prefectlegacy.cli.build_register`
    ...
    try:
        with prefectlegacy.context({"loading_flow": True, "local_script_path": path}):
...
\b    $ prefect run -m prefectlegacy.hello_world

\b  Run flow with a non-default parameter locally

\b    $ prefect run -m prefectlegacy.hello_world --param name=Marvin
...
def run(
    ...
    if path or module:
        ...
        with temporary_logger_config(
            ...
            with prefectlegacy.context(**context_dict):
                try:
