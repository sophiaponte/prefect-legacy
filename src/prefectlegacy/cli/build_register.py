...
from prefectlegacy.utilities.filesystems import read_bytes_from_path, parse_path
from prefectlegacy.utilities.graphql import with_args, EnumValue, compress
from prefectlegacy.utilities.importtools import import_object
from prefectlegacy.storage import Local, Module
from prefectlegacy.run_configs import UniversalRun


FlowLike = Union[box.Box, "prefectlegacy.Flow"]
...
def load_flows_from_script(path: str) -> "List[prefectlegacy.Flow]":
    ...
    try:
        with prefectlegacy.context({"loading_flow": True, "local_script_path": abs_path}):
    ...
    flows = [f for f in namespace.values() if isinstance(f, prefectlegacy.Flow)]
    if flows:
        for f in flows:
...
def load_flows_from_module(name: str) -> "List[prefectlegacy.Flow]":
    """
    ...
    if isinstance(mod_or_obj, ModuleType):
        flows = [f for f in vars(mod_or_obj).values() if isinstance(f, prefectlegacy.Flow)]
    elif isinstance(mod_or_obj, prefectlegacy.Flow):
        flows = [mod_or_obj]
...
def get_project_id(client: "prefectlegacy.Client", project: str) -> str:
...
def register_serialized_flow(
    ...
    Args:
        - client (prefectlegacy.Client): the prefect client
...
def build_and_register(
    ...
    Args:
        - client (prefectlegacy.Client): the prefect client to use
    ...
    for flow in flows:
        storage = flow.storage if isinstance(flow, prefectlegacy.Flow) else None
