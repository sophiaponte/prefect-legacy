...
from prefectlegacy import Client, config
from prefectlegacy.exceptions import AuthorizationError, ClientError
from prefectlegacy.cli.build_register import handle_terminal_error, TerminalError
...
def login(key):
    ...
    abort_on_config_api_key(
        "To log in with the CLI, remove the config key `prefectlegacy.cloud.api_key`"
    )
...
def logout(token):
    ...
    if client.api_key and not token:
        ...
        abort_on_config_api_key(
            "To log out, remove the config key `prefectlegacy.cloud.api_key`"
        )
...
def create_key(name, expire, quiet):
    ...
    if not user_id:
        raise TerminalError("Failed to retrieve the current user id from prefectlegacy Cloud")
    ...
    if key is None:
        raise TerminalError(f"Unexpected response from prefectlegacy Cloud: {response}")
    ...
    if config.cloud.get("auth_token"):
        if os.environ.get("PREFECT__CLOUD__AUTH_TOKEN"):
            ...
            click.secho(
                ...
                "ignored. We recommend removing the 'prefectlegacy.cloud.auth_token' key",
                fg="yellow",
            )
