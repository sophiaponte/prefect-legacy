...
from prefectlegacy.serialization.run_config import RunConfigSchema
from prefectlegacy.serialization.schedule import ScheduleSchema
from prefectlegacy.serialization.storage import StorageSchema
...
def get_parameters(obj: prefectlegacy.Flow, context: dict) -> List:
    if isinstance(obj, prefectlegacy.Flow):
...
def get_reference_tasks(obj: prefectlegacy.Flow, context: dict) -> List:
    if isinstance(obj, prefectlegacy.Flow):
...
def get_tasks(obj: prefectlegacy.Flow, context: dict) -> List:
    if isinstance(obj, prefectlegacy.Flow):
        tasks = obj.tasks
    else:
...
def get_edges(obj: prefectlegacy.Flow, context: dict) -> List:
    if isinstance(obj, prefectlegacy.Flow):
        edges = obj.edges
    else:
...
class FlowSchema(ObjectSchema):
    ...
    class Meta:
        object_class = lambda: prefectlegacy.core.Flow
    ...
    @post_load
    def create_object(self, data: dict, **kwargs: Any) -> prefectlegacy.core.Flow:
        """
