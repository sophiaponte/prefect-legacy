...
class State:
    ...
    **Note:** Each state-checking method (e.g., `is_failed()`) will also return `True`
    ...
    def __init__(
        ...
        if "task_tags" in prefectlegacy.context:
            self.context.setdefault("tags", list(prefectlegacy.context.task_tags))
    ...
    def __getstate__(self) -> dict:
        ...
        if isinstance(self.result, Exception) and not isinstance(
            self.result, prefectlegacy.engine.signals.PrefectStateSignal
        ):
...
class Scheduled(Pending):
    ...
    def __init__(
        ...
        run_count = prefectlegacy.context.get("task_run_count")
...
class Looped(Finished):
    ...
    def __init__(
        ...
        if loop_count is None:
            loop_count = prefectlegacy.context.get("task_loop_count", 1)
