...
def never_use(
    state: "prefectlegacy.engine.state.Cached",
    inputs: Dict[str, Any],
...
def partial_parameters_only(validate_on: Iterable[str] = None) -> Callable:
    ...
    from prefectlegacy.engine.cache_validators import partial_parameters_only
    ...
    ## the second run will use the cache contained within prefectlegacy.context.caches
...
def partial_inputs_only(validate_on: Iterable[str] = None) -> Callable:
    ...
    from prefectlegacy.engine.cache_validators import partial_inputs_only
