...
from prefectlegacy.engine.results import ConstantResult
...
from prefectlegacy.utilities import executors
from prefectlegacy.utilities.collections import flatten_seq
...
class FlowRunner(Runner):
    ...
    def __init__(
        ...
        if task_runner_cls is None:
            task_runner_cls = prefectlegacy.engine.get_default_task_runner_class()
    ...
    def run(
        ...
        run_context = dict(prefectlegacy.context)
        ...
        if executor is None:
            ...
            if executor is None:
                executor = prefectlegacy.engine.get_default_executor_class()()
        ...
        try:
            ...
            with prefectlegacy.context(run_context):
    ...
    def get_flow_run_state(
        ...
        executor: "prefectlegacy.executors.base.Executor",
    ) -> State:
        """
        Runs the flow.
        ...
        with self.check_for_cancellation(), executor.start():
            ...
            for task in self.flow.sorted_tasks():
                ...
                if task_state is None and isinstance(
                    task, prefectlegacy.tasks.core.constants.Constant
                ):
                ...
                if (
                    isinstance(
                        task,
                        (
                            prefectlegacy.tasks.core.resource_manager.ResourceSetupTask,
                            prefectlegacy.tasks.core.resource_manager.ResourceCleanupTask,
                            prefectlegacy.tasks.secrets.SecretBase,
                        ),
                    )
                ...
                for key, val in self.flow.constants[task].items():
                    edge = Edge(
                        upstream_task=prefectlegacy.tasks.core.constants.Constant(val),
                ...
                if any(edge.mapped for edge in upstream_states.keys()):
                    ...
                    task_states[task] = executor.wait(
                        executor.submit(
                            ...
                            context=dict(
                                prefectlegacy.context, **task_contexts.get(task, {})
                            ),
                    ...
                    task_states[task] = executor.submit(
                        ...
                        state=task_state,
                        upstream_states=upstream_states,
                        context=dict(prefectlegacy.context, **task_contexts.get(task, {})),
                        flow_result=self.flow.result,
                        task_runner_cls=self.task_runner_cls,
                        task_runner_state_handlers=task_runner_state_handlers,
                        upstream_mapped_states=upstream_mapped_states,
