...
from prefectlegacy.core import Edge, Task
from prefectlegacy.engine import signals
from prefectlegacy.engine.result.base import Result, ResultNotImplementedError
from prefectlegacy.engine.runner import ENDRUN, Runner, call_state_handlers
...
class TaskRunner(Runner):
    ...
    def __init__(
        ...
        self.context = prefectlegacy.context.to_dict()
        self.task = task
    ...
    def call_runner_target_handlers(self, old_state: State, new_state: State) -> State:
        ...
        self.logger.debug(
            "Task '{name}': Handling state change from {old} to {new}".format(
                name=prefectlegacy.context.get("task_full_name", self.task.name),
    ...
    def initialize_run(  # type: ignore
        ...
        if isinstance(map_index, int) and context.get("task_full_name"):
            context.update(
                logger=prefectlegacy.utilities.logging.get_logger(
    ...
    def run(
        ...
        context = context or prefectlegacy.context.to_dict()
        ...
        try:
            ...
            with prefectlegacy.context(context):

                if prefectlegacy.context.get("task_loop_count") is None:
                    self.logger.info(
        ...
        # task_inputs are set
            ...
            if prefectlegacy.context.get("raise_on_exception"):
                raise exc
        ...
        if prefectlegacy.context.get("task_loop_count") is None:
            # wrapping this final log in prefectlegacy.context(context) ensures
    ...
    def check_task_is_ready(self, state: State) -> State:
        ...
        # are generated
            ...
            self.logger.debug(
                ...
                prefectlegacy.context.get("task_full_name", self.task.name),
            )
            return state
    ...
    def set_task_run_name(self, task_inputs: Dict[str, Result]) -> None:
        """
        Sets the name for this task run and adds to `prefectlegacy.context`
        ...
        if task_run_name:
            ...
            formatting_kwargs = {
                **prefectlegacy.context.get("parameters", {}),
                **prefectlegacy.context,
                **raw_inputs,
            }
            ...
            prefectlegacy.context.update({"task_run_name": task_run_name})
    ...
    def check_target(self, state: State, inputs: Dict[str, Result]) -> State:
        ...
        if result and target:
            ...
            formatting_kwargs = {
                **prefectlegacy.context.get("parameters", {}).copy(),
                **prefectlegacy.context,
                **raw_inputs,
            }
    ...
    def check_task_is_cached(self, state: State, inputs: Dict[str, Result]) -> State:
        ...
        if state.is_cached():
            ...
            if self.task.cache_validator(
                state, sanitized_inputs, prefectlegacy.context.get("parameters")
            ):
                return state
            else:
        ...
        if self.task.cache_for is not None:
            ...
            if prefectlegacy.context.get("caches"):
                candidate_states = prefectlegacy.context.caches.get(
                    self.task.cache_key or self.task.name, []
                )
            sanitized_inputs = {key: res.value for key, res in inputs.items()}
            for candidate in candidate_states:
                if self.task.cache_validator(
                    candidate, sanitized_inputs, prefectlegacy.context.get("parameters")
                ):
                    return candidate
    ...
    def get_task_run_state(self, state: State, inputs: Dict[str, Result]) -> State:
        ...
        Raises:
            ...
            - ENDRUN: if the task is not ready to run
        """
        task_name = prefectlegacy.context.get("task_full_name", self.task.name)
        ...
        try:
            ...
            log_context = (
                redirect_stdout(prefectlegacy.utilities.logging.RedirectToLog(self.logger))
                if getattr(self.task, "log_stdout", False)
                else nullcontext()
            )  # type: AbstractContextManager

            with log_context:
                value = prefectlegacy.utilities.executors.run_task_with_timeout(
                    task=self.task,
                    args=(),
                    kwargs=raw_inputs,
                    logger=self.logger,
        ...
        if (
            prefectlegacy.context.get("checkpointing") is True
    ...
    def cache_result(self, state: State, inputs: Dict[str, Result]) -> State:
        ...
        if (
            ...
            cached_state = Cached(
                ...
                cached_parameters=prefectlegacy.context.get("parameters"),
    ...
    def check_for_retry(self, state: State, inputs: Dict[str, Result]) -> State:
        ...
        if state.is_failed():
            ...
            run_count = prefectlegacy.context.get("task_run_count", 1)
            loop_result = None
            state_context = None
            if prefectlegacy.context.get("task_loop_count") is not None:

                loop_result = self.result.from_value(
                    value=prefectlegacy.context.get("task_loop_result")
                )
                ...
                state_context = {"_loop_count": prefectlegacy.context["task_loop_count"]}
    ...
    def check_task_is_looping(
        ...
        if state.is_looped():
            ...
            context.update(task_run_version=prefectlegacy.context.get("task_run_version"))
