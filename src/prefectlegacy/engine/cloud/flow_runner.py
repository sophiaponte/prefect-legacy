...
from prefectlegacy.engine.flow_runner import FlowRunner, FlowRunnerInitializeResult
from prefectlegacy.engine.runner import ENDRUN
from prefectlegacy.engine.state import Failed, Queued, State, Cancelling, Cancelled
...
class CloudFlowRunner(FlowRunner):
    ...
    def _heartbeat(self) -> bool:
        try:
            ...
            self.flow_run_id = prefectlegacy.context.get("flow_run_id", "")  # type: str
    ...
    def call_runner_target_handlers(self, old_state: State, new_state: State) -> State:
        ...
        Returns:
            - State: the new state
        """
        raise_on_exception = prefectlegacy.context.get("raise_on_exception", False)
        ...
        flow_run_id = prefectlegacy.context.get("flow_run_id", None)
        version = prefectlegacy.context.get("flow_run_version")
        ...
        prefectlegacy.context.update(flow_run_version=(version or 0) + 1)
    ...
    def check_for_cancellation(self) -> Iterator:
        ...
        context = prefectlegacy.context.to_dict()
        ...
        def interrupt_if_cancelling() -> None:
            ...
            with prefectlegacy.context(context):
                flow_run_id = prefectlegacy.context["flow_run_id"]
                while True:
                    exiting_context = done.wait(
                        prefectlegacy.config.cloud.check_cancellation_interval
                    )
                    try:
        ...
        try:
            ...
            thread.join()
            if cancelling:
                prefectlegacy.context.update(flow_run_version=flow_run_version)
    ...
    def run(
        ...
        executor: "prefectlegacy.executors.Executor" = None,
        ...
        Args:
            ...
            - context (Dict[str, Any], optional): prefectlegacy.Context to use for execution
        ...
        while end_state.is_queued() and (
            ...
            while time_remaining > 0:
                delay = min(
                    prefectlegacy.config.cloud.check_cancellation_interval, time_remaining
                )
    ...
    def initialize_run(  # type: ignore
        ...
        try:
            ...
            if state is None:
                state = Failed(
                    message="Could not retrieve state from prefectlegacy Cloud", result=exc
                )
            raise ENDRUN(state=state) from exc
