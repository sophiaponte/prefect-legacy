...
from prefectlegacy.engine.runner import ENDRUN, call_state_handlers
from prefectlegacy.engine.state import Cached, ClientFailed, Failed, Queued, Retrying, State
from prefectlegacy.engine.task_runner import TaskRunner, TaskRunnerInitializeResult
from prefectlegacy.exceptions import VersionLockMismatchSignal
from prefectlegacy.utilities.executors import tail_recursive
...
class CloudTaskRunner(TaskRunner):
    ...
    def call_runner_target_handlers(self, old_state: State, new_state: State) -> State:
        ...
        except prefectlegacy.engine.signals.PrefectStateSignal as exc:
            self.logger.info(
        ...
        task_run_id = prefectlegacy.context.get("task_run_id")
        version = prefectlegacy.context.get("task_run_version")
        ...
        prefectlegacy.context.update(task_run_version=(version or 0) + 1)
    ...
    def initialize_run(  # type: ignore
        ...
        try:
            ...
            if state is None:
                state = Failed(
                    message="Could not retrieve state from prefectlegacy Cloud",
                    result=exc,
                )
            raise ENDRUN(state=state) from exc
    ...
    def check_task_is_cached(self, state: State, inputs: Dict[str, Result]) -> State:
        ...
        if self.task.cache_for is not None:
            ...
            cached_states = self.client.get_latest_cached_states(
                task_id=prefectlegacy.context.get("task_id", ""),
    ...
    def set_task_run_name(self, task_inputs: Dict[str, Result]) -> None:
        ...
        task_run_name = prefectlegacy.context.get("task_run_name")
