...
try:
    ...
    raise ImportError(
        "Using `prefectlegacy.tasks.templates.jinja2` requires Prefect to be installed "
    ...
    def run(self, template: str = None, **format_kwargs: Any) -> str:
        ...
        Args:
            ...
            - **format_kwargs (optional): keyword arguments to use for
                rendering; note that variables from `prefectlegacy.context` will also be used

        Returns:
            - str: the rendered string
        """
        template = Template(template)
        with prefectlegacy.context(**format_kwargs) as data:
