...
import prefectlegacy
from prefectlegacy import Client, Task, task
from prefectlegacy.backend.artifacts import create_link_artifact
from prefectlegacy.backend.flow_run import FlowRunView, FlowView, watch_flow_run
from prefectlegacy.client import Client
from prefectlegacy.engine.signals import signal_from_state
from prefectlegacy.run_configs import RunConfig
from prefectlegacy.utilities.graphql import EnumValue, with_args
...
def create_flow_run(
    ...
    if not run_name:
        current_run = prefectlegacy.context.get("flow_run_name")
        if current_run:
    ...
    if idempotency_key is None:
        ...
        # to `None`
        idempotency_key = prefectlegacy.context.get("task_run_id")
...
def get_task_run_result(
    ...
    if prefectlegacy.context.get("flow_run_id") == flow_run_id:
...
def wait_for_flow_run(
    ...
    for log in watch_flow_run(
        ...
        prefectlegacy.context.logger.log(log.level, message)
...
class StartFlowRun(Task):
    ...
    def run(
        ...
        if idempotency_key is None:
            idempotency_key = prefectlegacy.context.get("task_run_id", None)
