...
from prefectlegacy.exceptions import PrefectException
...
def _handle_databricks_task_execution(task, hook, log, submitted_run_id):
    ...
    Args:
        - task (prefectlegacy.Task) : Prefect task being handled
        - hook (prefectlegacy.tasks.databricks.databricks_hook.DatabricksHook): Databricks Hook
...
class DatabricksSubmitRun(Task):
    ...
    Examples:
        ...
        json = {
            ...
            'notebook_task': {
                'notebook_path': '/Users/andrew.h@prefectlegacy.io/records',
            },
        }
...
class DatabricksSubmitMultitaskRun(Task):
    ...
    tasks are distinct from prefectlegacy tasks. All tasks configured will run as a single Prefect task.
    ...
    def run(
        ...
        run_name = (
            run_name
            or f"Job run created by Prefect flow run {prefectlegacy.context.flow_run_name}"
        )
        # Ensures that multiple job runs are not created on retries
        idempotency_token = idempotency_token or prefectlegacy.context.flow_run_id
