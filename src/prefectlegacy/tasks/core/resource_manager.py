...
class ResourceContext:
    ...
    def __enter__(self) -> Optional[Task]:
        self.__prev_resource = prefectlegacy.context.get("resource")
        prefectlegacy.context.update(resource=self)
        return self.setup_task

    def __exit__(self, *args: Any) -> None:
        if self.__prev_resource is None:
            prefectlegacy.context.pop("resource", None)
        else:
            prefectlegacy.context.update(resource=self.__prev_resource)
...
class ResourceManager:
    ...
    def __call__(self, *args: Any, flow: Flow = None, **kwargs: Any) -> ResourceContext:
        if flow is None:
            flow = prefectlegacy.context.get("flow")
            if flow is None:
