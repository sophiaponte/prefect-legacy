...
def execute_flow_run(
    flow_run_id: str,
    flow: "Flow" = None,
    runner_cls: Type["prefectlegacy.engine.flow_runner.FlowRunner"] = None,
    **kwargs: Any,
) -> "FlowRunView":
    """
    ...
    runner_cls = runner_cls or prefectlegacy.engine.cloud.flow_runner.CloudFlowRunner
    ...
    if flow_metadata.storage:
        ...
        for secret in flow_metadata.storage.secrets:
            with _fail_flow_run_on_exception(
                ...
                secrets[secret] = prefectlegacy.tasks.secrets.PrefectSecret(name=secret).run()
...
def generate_flow_run_environ(
    ...
    client = prefectlegacy.Client()  # Instantiate a client to get the current API key
...
def _fail_flow_run(flow_run_id: str, message: str) -> None:
    ...
    client.set_flow_run_state(
        flow_run_id=flow_run_id, state=prefectlegacy.engine.state.Failed(message)
    )
