...
from prefectlegacy.executors import Executor
from prefectlegacy.engine.result import Result
from prefectlegacy.engine.state import State
from prefectlegacy.storage import Storage, get_default_storage_class
from prefectlegacy.run_configs import RunConfig, UniversalRun
from prefectlegacy.utilities import diagnostics, logging
...
class Flow:
    ...
    Args:
        ...
        - schedule (prefectlegacy.schedules.Schedule, optional): A default schedule for the flow
        - executor (prefectlegacy.executors.Executor, optional): The executor that the flow
           should use. If `None`, the default executor configured in the runtime environment
           will be used.
        - run_config (prefectlegacy.run_configs.RunConfig, optional): The runtime
           configuration to use when deploying this flow.
        - storage (prefectlegacy.storage.Storage, optional): The unit of storage
    ...
    def __init__(
        self,
        name: str,
        schedule: prefectlegacy.schedules.Schedule = None,
        ...
        self._prefect_version = prefectlegacy.__version__
    ...
    def _flow_context(self) -> Iterator["Flow"]:
        ...
        with prefectlegacy.context(flow=self, _unused_task_tracker=unused_task_tracker):
            yield self
        ...
        if unused_task_tracker.difference(self.tasks):
            warnings.warn(
                ...
                "https://docs.prefectlegacy.io/core/advanced_tutorials/"
    ...
    def add_task(self, task: Task) -> Task:
        ...
        if not isinstance(task, Task):
            ...
            if not isinstance(task, (Parameter, prefectlegacy.tasks.core.constants.Constant)):
                case = prefectlegacy.context.get("case", None)
                if case is not None:
                    case.add_task(task, self)
                resource = prefectlegacy.context.get("resource", None)
    ...
    def add_edge(
        ...
        Returns:
            - prefectlegacy.core.edge.Edge: The `Edge` object that was successfully added to the flow
        ...
        """
        if validate is None:
            validate = cast(bool, prefectlegacy.config.flows.eager_edge_validation)

        if mapped and prefectlegacy.context.get("mapped", False):
            raise ValueError(
        ...
        if (
            isinstance(edge.upstream_task, prefectlegacy.tasks.core.constants.Constant)
            and edge.key
            and not edge.mapped
    ...
    def set_dependencies(
        ...
        Args:
            ...
            - mapped (bool, optional): Whether the upstream tasks (both keyed
                ...
                tasks wrapped in the `prefectlegacy.utilities.edges.unmapped` container will
    ...
    def _run(
        ...
        **kwargs: Any,
        ...
        flow_state = prefectlegacy.engine.state.Scheduled(start_time=next_run_time, result={})
        ...
        while True:
            ...
            while not flow_state.is_finished():
                ...
                for t, s in filter(
                    lambda tup: isinstance(tup[1], prefectlegacy.engine.state.Paused),
                    flow_state.result.items(),
                ):
                    approve = input(f"{t} is currently Paused; enter 'y' to resume:\n")
                    if approve.strip().lower() == "y":
                        flow_state.result[t] = prefectlegacy.engine.state.Resume(
            ...
            if not error:
                ...
                for t, s in flow_state.result.items():
                    ...
                    fresh_states = [
                        s
                        for s in prefectlegacy.context.caches.get(t.cache_key or t.name, [])
                        + cached_sub_states
                        if s.cached_result_expiration
                        and s.cached_result_expiration > now
                    ]
                    prefectlegacy.context.caches[t.cache_key or t.name] = fresh_states
            ...
            flow_state = prefectlegacy.engine.state.Scheduled(
    ...
    def run(
        ...
        **kwargs: Any,
    ) -> Union["prefectlegacy.engine.state.State", None]:
        """
        ...
        if prefectlegacy.context.get("loading_flow", False):
            warnings.warn(
        ...
        if runner_cls is None:
            runner_cls = prefectlegacy.engine.get_default_flow_runner_class()
        ...
        prefectlegacy.context.setdefault("caches", {})

        if run_on_schedule is None:
            run_on_schedule = cast(bool, prefectlegacy.config.flows.run_on_schedule)
        ...
        for task in self.tasks or []:
            if task not in state.result:
                state.result[task] = prefectlegacy.engine.state.Pending(
    ...
    def visualize(
        self,
        flow_state: "prefectlegacy.engine.state.State" = None,
        filename: str = None,
        format: str = None,
    ...
    def serialize(self, build: bool = False) -> dict:
        ...
        self.validate()
        schema = prefectlegacy.serialization.flow.FlowSchema
    ...
    def load(cls, fpath: str) -> "Flow":
        ...
        """
        from prefectlegacy.utilities.storage import flow_from_bytes_pickle

        if not os.path.isabs(fpath):
            path = "{home}/flows".format(home=prefectlegacy.context.config.home_dir)  # type: ignore
    ...
    def run_agent(
        ...
        """
        temp_config = {
            "cloud.agent.auth_token": token or prefectlegacy.config.cloud.agent.auth_token,
            "cloud.api_key": api_key or prefectlegacy.config.cloud.get("api_key"),
            "cloud.send_flow_run_logs": (
                log_to_cloud
                if log_to_cloud is not None
                else prefectlegacy.config.cloud.send_flow_run_logs
            ),
        }
        with set_temporary_config(temp_config):
            if self.run_config is not None:
                ...
                labels = []
            agent = prefectlegacy.agent.local.LocalAgent(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            - labels (List[str], optional): a list of labels to add to this Flow;
                ...
                http://docs.prefectlegacy.io/orchestration/agents/overview.html#labels
