...
class Edge:
    ...
    Args:
        - upstream_task (Any): the task that must run before the
            ...
            `prefectlegacy.utilities.tasks.as_task()`
        ...
        - flow (prefectlegacy.Flow, optional): a flow object that will be used
    ...
    def __init__(
        ...
        mapped: bool = None,
        flattened: bool = None,
        flow: "prefectlegacy.core.flow.Flow" = None,
    ):
        ...
        annotations = {}
        if isinstance(upstream_task, prefectlegacy.utilities.edges.EdgeAnnotation):
            annotations.update(upstream_task.annotations)
        if isinstance(downstream_task, prefectlegacy.utilities.edges.EdgeAnnotation):
            annotations.update(downstream_task.annotations)
        upstream_task = prefectlegacy.utilities.tasks.as_task(upstream_task, flow=flow)
        downstream_task = prefectlegacy.utilities.tasks.as_task(downstream_task, flow=flow)
    ...
    def serialize(self) -> dict:
        ...
        """
        return prefectlegacy.serialization.edge.EdgeSchema().dump(self)
