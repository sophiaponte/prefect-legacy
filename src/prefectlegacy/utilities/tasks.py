...
if TYPE_CHECKING:
    import prefectlegacy.tasks.core.constants
    import prefectlegacy.tasks.core.collections
    import prefectlegacy.tasks.core.function
...
def apply_map(func: Callable, *args: Any, flow: "Flow" = None, **kwargs: Any) -> Any:
    ...
    `prefectlegacy.unmapped`. Nested arguments that should be unnested before mapping
    over should be wrapped with `prefectlegacy.flatten`
    ...
    ```python
    from prefectlegacy import task, case, apply_map
    ...
    assert isinstance(flow, prefectlegacy.Flow)  # appease mypy
    ...
    for x in itertools.chain(args, kwargs.values()):
        if not isinstance(
            x, (prefectlegacy.Task, prefectlegacy.utilities.edges.EdgeAnnotation, Sequence)
        ):
            raise TypeError(
                f"Cannot map over non-sequence object of type `{type(x).__name__}`"
            )

    flow2 = prefectlegacy.Flow("temporary flow")
    ...
    def preprocess(a: Any) -> "prefectlegacy.Task":
        # Clear external case/resource when adding tasks to flow2
        with prefectlegacy.context(case=None, resource=None):
            a2 = as_task(a, flow=flow2)
            is_mapped = not isinstance(a, prefectlegacy.utilities.edges.unmapped)
            is_constant = isinstance(a2, Constant)
            is_flattened = isinstance(a, prefectlegacy.utilities.edges.flatten)
            if not is_constant:
    ...
    with prefectlegacy.context(mapped=True, case=None, resource=None):
        with flow2:
            if no_flow_provided:
...
def tags(*tags: str) -> Iterator[None]:
    ...
    """
    tags_set = set(tags)
    tags_set.update(prefectlegacy.context.get("tags", set()))
    with prefectlegacy.context(tags=tags_set):
        yield


def as_task(x: Any, flow: "Optional[Flow]" = None) -> "prefectlegacy.Task":
    """
    ...
    def is_constant(x: Any) -> bool:
        ...
        """
        if isinstance(x, (prefectlegacy.core.Task, prefectlegacy.utilities.edges.EdgeAnnotation)):
            return False
    ...
    # task objects
    if isinstance(x, prefectlegacy.core.Task):  # type: ignore
        return x
    elif isinstance(x, prefectlegacy.utilities.edges.EdgeAnnotation):
        return as_task(x.value, flow=flow)

    # handle constants, including collections of constants
    elif is_constant(x):
        return_task = Constant(x)  # type: prefectlegacy.core.Task

    # collections
    elif isinstance(x, list):
        return_task = prefectlegacy.tasks.core.collections.List().bind(*x, flow=flow)
    elif isinstance(x, tuple):
        return_task = prefectlegacy.tasks.core.collections.Tuple().bind(*x, flow=flow)
    elif isinstance(x, set):
        return_task = prefectlegacy.tasks.core.collections.Set().bind(*x, flow=flow)
    elif isinstance(x, dict):
        keys, values = [], []
        for k, v in x.items():
            keys.append(k)
            values.append(v)
        return_task = prefectlegacy.tasks.core.collections.Dict().bind(
...
def pause_task(message: str = None, duration: timedelta = None) -> None:
    ...
    Example:
        ```python
        from prefectlegacy import Flow
        from prefectlegacy.utilities.tasks import task, pause_task
    ...
    if duration is not None:
        ...
        start_time = None
    if prefectlegacy.context.get("resume", False) is False:
        raise prefectlegacy.engine.signals.PAUSE(  # type: ignore
...
@overload
def task(__fn: Callable) -> "prefectlegacy.tasks.core.function.FunctionTask":
    pass
...
) -> Callable[[Callable], "prefectlegacy.tasks.core.function.FunctionTask"]:
    pass
...
def task(
    ...
    "prefectlegacy.tasks.core.function.FunctionTask",
    Callable[[Callable], "prefectlegacy.tasks.core.function.FunctionTask"],
]:
    """
    ...
    """
    if fn is None:
        return lambda fn: prefectlegacy.tasks.core.function.FunctionTask(
            fn=fn,
            **task_init_kwargs,
        )
    return prefectlegacy.tasks.core.function.FunctionTask(fn=fn, **task_init_kwargs)
