...
class LogManager:
    ...
    def __init__(self) -> None:
        ...
        self.client = None  # type: Optional[prefectlegacy.Client]
    ...
    def ensure_started(self) -> None:
        ...
        if self.thread is None:
            self.client = prefectlegacy.Client()
...
def get_logger(name: str = None) -> logging.Logger:
    ...
    Args:
        - name (str): if `None`, the root Prefect logger is returned. If provided, a child
            logger of the name `"prefectlegacy.{name}"` is returned. The child logger inherits
...
def temporary_logger_config(
    ...
    try:
        with prefectlegacy.utilities.configuration.set_temporary_config(overrides):
