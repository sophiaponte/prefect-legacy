...
from prefectlegacy.configuration import to_environment_variables
from prefectlegacy.exceptions import TaskTimeoutSignal, PrefectSignal
...
if TYPE_CHECKING:
    import prefectlegacy.engine.runner
...
def run_with_heartbeat(
    runner_method: Callable[..., "prefectlegacy.engine.state.State"]
) -> Callable[..., "prefectlegacy.engine.state.State"]:
    """
    ...
    @wraps(runner_method)
    def inner(
        self: "prefectlegacy.engine.cloud.CloudFlowRunner", *args: Any, **kwargs: Any
        ...
        if not use_heartbeat or (prefectlegacy.context.config.cloud.heartbeat_mode == "off"):
            configured_heartbeat = no_heartbeat()
        elif prefectlegacy.context.config.cloud.heartbeat_mode == "thread":
            configured_heartbeat = threaded_heartbeat(self.flow_run_id)
        elif prefectlegacy.context.config.cloud.heartbeat_mode == "process":
...
def send_heartbeat(
    flow_run_id: str, client: "prefectlegacy.client.Client", logger: "logging.Logger"
) -> None:
...
def run_with_thread_timeout(
    ...
    Args:
        ...
        - logger (Logger): an optional logger to use. If not passed, a logger for the
            `prefectlegacy.executors.run_with_thread_timeout` namespace will be created.
...
def run_with_multiprocess_timeout(
    ...
    Args:
        ...
        - timeout (int): the length of time to allow for execution before raising a
            `TaskTimeoutSignal`, represented as an integer in seconds
        - logger (Logger): an optional logger to use. If not passed, a logger for the
            `prefectlegacy.` namespace will be created.
        - name (str): an optional name to attach to logs for this function run, defaults
            to the name of the given function. Provides an interface for passing task
            names for logs.
...
def run_task_with_timeout(
    ...
    Args:
        ...
        - kwargs (Mapping): keyword arguments to pass to the function
        - logger (Logger): an optional logger to use. If not passed, a logger for the
            `prefectlegacy.run_task_with_timeout_handler` namespace will be created.
    ...
    """
    logger = logger or get_logger()
    name = prefectlegacy.context.get("task_full_name", task.name)
    kwargs = kwargs or {}
    ...
    fut = executor.submit(run_with_ctx, prefectlegacy.context.to_dict())
...
def prepare_upstream_states_for_mapping(
    ...
    executor: "prefectlegacy.executors.Executor",
) -> list:
    """
        ...
        try:
            ...
            for edge, upstream_state in upstream_states.items():
                ...
                # array.
                    ...
                    if (
                        not state.is_mapped()
                        or upstream_state._result != prefectlegacy.engine.result.NoResult
                    ):
