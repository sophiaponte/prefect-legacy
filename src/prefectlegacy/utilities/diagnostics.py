...
def system_information() -> dict:
    ...
    return dict(
        system_information=dict(
            ...
            prefect_version=prefectlegacy.__version__,
            prefect_backend=prefectlegacy.config.backend,
        )
    )
...
def config_overrides(include_secret_names: bool = False) -> dict:
    ...
    *not* returned.
    ...
    default_config = dict()
    default_config_path = prefectlegacy.configuration.DEFAULT_CONFIG
    if default_config_path and os.path.isfile(default_config_path):
        default_config = prefectlegacy.configuration.load_toml(default_config_path)

    user_config = dict()  # type: ignore
    user_config_path = prefectlegacy.configuration.USER_CONFIG
    if user_config_path and os.path.isfile(
        str(prefectlegacy.configuration.interpolate_env_vars(user_config_path))
    ):
        user_config = prefectlegacy.configuration.load_toml(user_config_path)

    # Create some shorter names for fully specified imports avoiding circular
    # dependencies in the utilities
    dict_to_flatdict = prefectlegacy.utilities.collections.dict_to_flatdict
    flatdict_to_dict = prefectlegacy.utilities.collections.flatdict_to_dict
...
def environment_variables() -> dict:
    ...
    for env_var, _ in os.environ.items():
        if env_var.startswith(prefectlegacy.configuration.ENV_VAR_PREFIX + "__"):
            env_vars.append(env_var)

    return dict(env_vars=env_vars)


def flow_information(flow: "prefectlegacy.Flow") -> dict:
    """
    Get flow information

    Args:
        - flow ("prefectlegacy.Flow"): the flow whose attributes to retrieve
...
def diagnostic_info(
    flow: "prefectlegacy.Flow" = None, include_secret_names: bool = False
) -> str:
    """
    Get full diagnostic information

    Args:
        - flow ("prefectlegacy.Flow", optional): the flow whose attributes to retrieve. If no
