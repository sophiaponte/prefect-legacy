...
if TYPE_CHECKING:
    from prefectlegacy.core.flow import Flow  # pylint: disable=W0611
...
def get_flow_image(flow: "Flow") -> str:
    ...
    This function was deprecated in 0.15.0 and `prefectlegacy.utilities.agent.get_flow_image`
    ...
    if run_config is not None and hasattr(run_config, "image"):
        ...
        if not isinstance(storage, prefectlegacy.storage.Docker):
            raise ValueError(
...
def extract_flow_from_file(
    ...
    flows = {o.name: o for o in exec_vals.values() if isinstance(o, prefectlegacy.Flow)}
    if flows:
        if flow_name:
...
def extract_flow_from_module(module_str: str, flow_name: str = None) -> "Flow":
    ...
    if obj_name_present:
        ...
        if callable(flow):
            flow = flow()
        if not isinstance(flow, prefectlegacy.Flow):
            raise TypeError(
                f"Object at {module_str!r} is a {type(flow)} not a `prefectlegacy.Flow`"
            )
...
def _get_versions() -> dict:
    ...
    return {
        ...
        "prefect": prefectlegacy.__version__,
