...
from prefectlegacy.exceptions import (
    AuthorizationError,
    ClientError,
    ObjectNotFoundError,
    VersionLockMismatchSignal,
)
from prefectlegacy.run_configs import RunConfig
...
from prefectlegacy.utilities.logging import create_diagnostic_logger

if TYPE_CHECKING:
    import requests

    from prefectlegacy.core import Flow
...
class TaskRunInfoResult(NamedTuple):
    # TODO: Deprecate this result in favor of `prefectlegacy.backend.TaskRun`
    id: str
    task_id: str
    task_slug: str
    version: int
...
class FlowRunInfoResult(NamedTuple):
    # TODO: Deprecate this result in favor of `prefectlegacy.backend.FlowRun`
    id: str
    name: str
    flow_id: str
    project: ProjectInfo
...
class Client:
    ...
    def __init__(
        ...
        self._auth_file = Path(prefectlegacy.context.config.home_dir).absolute() / "auth.toml"
        ...
        self.api_server = api_server or prefectlegacy.context.config.cloud.api

        # Load the API key
        cached_auth = self.load_auth_from_disk()
        self.api_key: Optional[str] = (
            api_key
            or prefectlegacy.context.config.cloud.get("api_key")
        ...
        self.tenant_id: Optional[str] = (
            ...
            or prefectlegacy.context.config.cloud.get("tenant_id")
    ...
    def _get_auth_tenant(self) -> str:
        ...
        """
        if not prefectlegacy.config.backend == "cloud":

            raise ValueError(
                "Authentication is only supported for Prefect Cloud. "
                "Your backend is set to {prefectlegacy.config.backend!r}"
            )
    ...
    def _get_default_server_tenant(self) -> str:
        if prefectlegacy.config.backend == "server":
        ...
        elif prefectlegacy.config.backend == "cloud":
            raise ValueError(
                "Default tenants are determined by authentication in Prefect Cloud. "
                "See `_get_auth_tenant` instead."
            )
        else:
            raise ValueError("Unknown backend {prefectlegacy.config.backend!r}")
    ...
    def tenant_id(self) -> str:
        ...
        if not self._tenant_id:
            if prefectlegacy.config.backend == "cloud":
                self._tenant_id = self._get_auth_tenant()
            elif prefectlegacy.config.backend == "server":
                self._tenant_id = self._get_default_server_tenant()
            else:
                raise ValueError(f"Unknown backend setting {prefectlegacy.config.backend!r}")
    ...
    def graphql(
        ...
        Args:
            - query (Any): A representation of a graphql query to be executed. It will be
                parsed by prefectlegacy.utilities.graphql.parse_graphql().
    ...
    def _send_request(
        ...
        if prefectlegacy.context.config.cloud.get("diagnostics") is True:
        ...
        if method == "GET":
            response = session.get(
                ...
                headers=headers,
                params=params,
                timeout=prefectlegacy.context.config.cloud.request_timeout,
            )
        elif method == "POST":
        ...
        # is still being hit.
        #
        # sleep/backoff times can be customized in prefectlegacy.config.cloud.rate_limiting
        rate_limited = response.status_code == 429
        if (
            rate_limited
            and rate_limit_counter <= prefectlegacy.config.cloud.rate_limiting.max_retries
        ):
            jitter = (
                random.random()
                * prefectlegacy.config.cloud.rate_limiting.backoff_multiplier
                * (
                    prefectlegacy.config.cloud.rate_limiting.backoff_exponent
                    ** rate_limit_counter
                )
            )
            naptime = prefectlegacy.config.cloud.rate_limiting.backoff_s + jitter
            self.logger.warning(
    ...
    def _request(
        ...
        headers["X-PREFECT-CORE-VERSION"] = str(prefectlegacy.__version__)
        ...
        try:
            ...
            if prefectlegacy.config.backend == "cloud" and "Authorization" not in headers:
                raise AuthorizationError(
    ...
    def register(
        ...
        try:
            prefectlegacy.serialization.flow.FlowSchema().load(serialized_flow)
    ...
    def get_cloud_url(self, subdirectory: str, id: str) -> str:
        ...
        # returns "https://cloud.prefectlegacy.io/my-tenant-slug/flow-run/424242-ca-94611-111-55"
        ```
        """

        # Search for matching cloud API because we can't guarantee that the backend config is set
        using_cloud_api = ".prefectlegacy.io" in prefectlegacy.config.cloud.api
        tenant_slug = self.get_default_tenant_slug()

        # For various API versions parse out `api-` for direct UI link
        base_url = (
            (
                re.sub("api-", "", prefectlegacy.config.cloud.api)
                if re.search("api-", prefectlegacy.config.cloud.api)
                else re.sub("api", "cloud", prefectlegacy.config.cloud.api)
            )
            if using_cloud_api
            else prefectlegacy.config.server.ui.endpoint
        )
    ...
    def get_flow_run_info(self, flow_run_id: str) -> FlowRunInfoResult:
        ...
        task_runs = [
            TaskRunInfoResult(
                ...
                else prefectlegacy.engine.state.Pending(),
            )
            for tr in result.task_runs
        ]

        state = (
            prefectlegacy.engine.state.State.deserialize(result.serialized_state)
    ...
    def get_flow_run_state(self, flow_run_id: str) -> "prefectlegacy.engine.state.State":
        """
        ...
        return prefectlegacy.engine.state.State.deserialize(flow_run.serialized_state)
    ...
    def set_flow_run_state(
        ...
        if state_payload.status == "QUEUED":
            ...
            return prefectlegacy.engine.state.Queued(
                message=state_payload.get("message"),
                start_time=pendulum.now("UTC").add(
                    seconds=prefectlegacy.context.config.cloud.queue_interval
                ),
            )
        return state
    ...
    def get_latest_cached_states(
    ...
    ) -> List["prefectlegacy.engine.state.State"]:
        """
        ...
        deserializer = prefectlegacy.engine.state.State.deserialize
        valid_states = [
    ...
    def get_task_run_info(
        ...
        state = (
            prefectlegacy.engine.state.State.deserialize(task_run_info.serialized_state)
    ...
    def get_task_run_state(self, task_run_id: str) -> "prefectlegacy.engine.state.State":
        """
        ...
        return prefectlegacy.engine.state.State.deserialize(task_run.serialized_state)
