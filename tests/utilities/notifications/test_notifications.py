...
from prefectlegacy.utilities.notifications import (
    callback_factory,
    gmail_notifier,
...
def test_slack_notifier_returns_new_state_and_old_state_is_ignored(monkeypatch):
    ...
    with set_temporary_config({"cloud.use_local_secrets": True}):
        with prefectlegacy.context(secrets=dict(SLACK_WEBHOOK_URL="")):
...
def test_slack_notifier_pulls_url_from_secret(monkeypatch):
    ...
    with set_temporary_config({"cloud.use_local_secrets": True}):
        ...
        with prefectlegacy.context(secrets=dict(SLACK_WEBHOOK_URL="https://foo/bar")):
            slack_notifier(Task(), "", state)

        assert post.call_args[0][0] == "https://foo/bar"

        with prefectlegacy.context(secrets=dict(TOP_SECRET='"42"')):
...
def test_gmail_notifier_sends_simple_email(monkeypatch):
    ...
    monkeypatch.setattr(prefectlegacy.utilities.notifications.notifications, "smtplib", smtp)
    s = Failed("optional message...")

    monkeypatch.setattr(prefectlegacy.config.cloud, "use_local_secrets", True)
    with prefectlegacy.context(secrets=dict(EMAIL_USERNAME="alice", EMAIL_PASSWORD=1234)):
        returned = gmail_notifier(Task(name="dud"), "", s)

    assert returned is s
    email_from, to, body = sendmail.call_args[0]
    assert email_from == "notifications@prefectlegacy.io"
    assert to == "alice"
...
def test_gmail_notifier_is_curried_and_uses_only_states(monkeypatch, state):
    ...
    monkeypatch.setattr(prefectlegacy.utilities.notifications.notifications, "smtplib", smtp)
    with set_temporary_config({"cloud.use_local_secrets": True}):
        with prefectlegacy.context(secrets=dict(EMAIL_USERNAME="", EMAIL_PASSWORD="")):
