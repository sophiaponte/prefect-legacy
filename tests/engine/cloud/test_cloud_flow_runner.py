...
from prefectlegacy.engine.runner import ENDRUN
...
from prefectlegacy.exceptions import VersionLockMismatchSignal
...
def client(monkeypatch):
    ...
    monkeypatch.setattr(
        "prefectlegacy.engine.cloud.task_runner.Client", MagicMock(return_value=cloud_client)
    )
    monkeypatch.setattr(
        "prefectlegacy.engine.cloud.flow_runner.Client", MagicMock(return_value=cloud_client)
    )
    yield cloud_client


def test_task_runner_cls_is_cloud_task_runner():
    fr = CloudFlowRunner(flow=prefectlegacy.Flow(name="test"))
...
def test_flow_runner_raises_endrun_if_client_cant_update_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "prefectlegacy.engine.cloud.flow_runner.Client", MagicMock(return_value=client)
    )
...
def test_client_is_always_called_even_during_state_handler_failures(client):
    ...
    flow = prefectlegacy.Flow(name="test", tasks=[prefectlegacy.Task()], state_handlers=[handler])
...
def test_flow_handlers_are_called_even_when_initialize_run_fails(client):
    ...
    flow = prefectlegacy.Flow(name="test", state_handlers=[handler])
    with prefectlegacy.context(flow_run_version=0):
...
def test_flow_runner_puts_running_with_backend_in_context(client):
    @prefectlegacy.task()
    def whats_in_ctx():
        assert prefectlegacy.context.get("running_with_backend")

    flow = prefectlegacy.Flow(name="test", tasks=[whats_in_ctx])
    res = CloudFlowRunner(flow=flow).run()
...
def test_heartbeat_traps_errors_caused_by_client(caplog, monkeypatch, heartbeat):
    with set_temporary_config({"cloud.heartbeat_mode": heartbeat}):
        ...
        runner = CloudFlowRunner(flow=prefectlegacy.Flow(name="bad"))
...
def test_flow_runner_heartbeat_sets_command(monkeypatch, setting_available, heartbeat):
    with set_temporary_config({"cloud.heartbeat_mode": heartbeat}):
        ...
        runner = CloudFlowRunner(flow=prefectlegacy.Flow(name="test"))
        with prefectlegacy.context(flow_run_id="foo"):
            res = runner._heartbeat()
...
def test_state_handler_failures_are_handled_appropriately(client):
    ...
    f = prefectlegacy.Flow(name="test", tasks=[do_nothing], on_failure=bad)
...
def test_starting_at_arbitrary_loop_index_from_cloud_context(client):
    ...
    with prefectlegacy.Flow(name="looping", result=PrefectResult()) as f:
        inter = looper(10)
...
def test_cloud_task_runners_submitted_to_remote_machines_respect_original_config(
    ...
    cluster which has _not_ been configured for prefectlegacy.  The idea is that the configuration
    ...
    """
    with set_temporary_config({"cloud.heartbeat_mode": heartbeat}):
        from prefectlegacy.engine.flow_runner import run_task
        ...
        monkeypatch.setattr("prefectlegacy.engine.flow_runner.run_task", my_run_task)
        monkeypatch.setattr("prefectlegacy.client.Client", Client)
        monkeypatch.setattr("prefectlegacy.engine.cloud.task_runner.Client", Client)
        monkeypatch.setattr("prefectlegacy.engine.cloud.flow_runner.Client", Client)
        monkeypatch.setattr("prefectlegacy.utilities.executors.Client", MagicMock())
        monkeypatch.setattr("prefectlegacy.cli.heartbeat.Client", MagicMock())
        prefectlegacy.utilities.logging.prefect_logger.handlers[-1].client = Client()

        @prefectlegacy.task(result=PrefectResult())
        def log_stuff():
            logger = prefectlegacy.context.get("logger")
            logger.critical("important log right here")
            return (
                prefectlegacy.context.config.special_key,
                prefectlegacy.context.config.cloud.api_key,
            )
        ...
        with prefectlegacy.utilities.configuration.set_temporary_config(
            ...
            flow = prefectlegacy.Flow("test", tasks=[log_stuff])
    ...
    logs = [
        c[0][0] for c in prefectlegacy.utilities.logging.LOG_MANAGER.enqueue.call_args_list
    ]
...
class TestCloudFlowRunnerQueuedState:
    ...
    def do_mocked_run(
        ...
        monkeypatch.setattr("prefectlegacy.engine.cloud.flow_runner.FlowRunner.run", mock_run)
        monkeypatch.setattr("prefectlegacy.engine.cloud.flow_runner.time_sleep", mock_sleep)

        @prefectlegacy.task
        def return_one():
            return 1

        with prefectlegacy.Flow("test-cloud-flow-runner-with-queues") as flow:
            return_one()
...
class TestCloudFlowRunnerCancellation:
    ...
    def test_check_interrupt_loop_logging_calls_have_orig_prefect_context(
        ...
        def emit(*args, **kwargs):
            calls.append(prefectlegacy.context.config.get("special_key") == 42)
        ...
        with set_temporary_config(
            ...
            monkeypatch.setattr("prefectlegacy.utilities.logging.CloudHandler.emit", emit)
