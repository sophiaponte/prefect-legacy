...
class MockClient:
    ...
    def pipeline_runs(self):
        return MagicMock(
            create_or_update=lambda *args: MockFactory(),
            get=lambda *args: MagicMock(status="Succeeded"),
        )
...
class TestDatafactoryCreate:
    ...
    @pytest.mark.parametrize(
        "required_arg", ["datafactory_name", "resource_group_name"]
    )
...
class TestPipelineCreate:
    ...
    @pytest.mark.parametrize(
        "required_arg",
        ["datafactory_name", "resource_group_name", "pipeline_name", "activities"],
    )
    def test_raises_if_required_args_not_eventually_provided(self, required_arg):
        task = PipelineCreate()
        required_args = {
            "datafactory_name": "name",
...
class TestPipelineRun:
    ...
    @pytest.mark.parametrize(
        "required_arg", ["datafactory_name", "resource_group_name", "pipeline_name"]
    )
    def test_raises_if_required_args_not_eventually_provided(self, required_arg):
        task = PipelineRun()
        required_args = {
