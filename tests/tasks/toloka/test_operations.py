...
def assignments_df_mock():
    return pd.DataFrame.from_records(
        [{"ASSIGNMENT:assignment_id": f"assignment-{index}"} for index in range(3)]
    )
...
class TestCreateExamPool:
    ...
    def test_create_exam_pool_with_project(
        self, toloka_client_mock, training_mock, project_mock
    ):
...
class TestCreatePool:
    ...
    def test_create_pool_with_project(
        self, toloka_client_mock, pool_mock, project_mock
    ):
    ...
    def test_create_pool_with_exam(
        self, toloka_client_mock, pool_mock, project_mock, training_mock
    ):
        ...
        res = create_pool.run(
            pool_mock.unstructure(), project_id=project_mock, exam_pool_id=training_mock
        )
        assert (
            training_mock.id
            == res.quality_control.training_requirement.training_pool_id
        )
    ...
    def test_create_pool_with_exam_error(
        ...
        pool_mock.quality_control = QualityControl()
        with pytest.raises(
            ValueError, match="pool.quality_control.training_requirement"
        ):
            create_pool.run(
                pool_mock.unstructure(),
    ...
    def test_create_pool_with_expiration_relative(self, toloka_client_mock, pool_mock):
        with freeze_time(datetime(2022, 1, 1, tzinfo=pytz.UTC)):
            res = create_pool.run(
                pool_mock.unstructure(), expiration=timedelta(days=31)
            )
        assert datetime(2022, 2, 1, tzinfo=pytz.UTC) == res.will_expire.replace(
            tzinfo=pytz.UTC
        )
...
class TestCreateTasks:
    def test_create_tasks(self, toloka_client_mock, tasks_mock, tasks_creation_result):
        ...
        assert tasks_creation_result == create_tasks.run(
            [task.unstructure() for task in tasks_mock], **kwargs
        )
    ...
    def test_create_tasks_with_pool(
        self, toloka_client_mock, tasks_mock, tasks_creation_result, pool_mock
    ):
        assert tasks_creation_result == create_tasks.run(
    ...
    def test_create_tasks_with_exam_pool(
        self, toloka_client_mock, tasks_mock, tasks_creation_result, training_mock
    ):
        assert tasks_creation_result == create_tasks.run(
            [task.unstructure() for task in tasks_mock],
...
class TestGetAssignments:
    def test_get_assignments_by_pool_object(
        self, toloka_client_mock, pool_mock, assignments_mock
    ):
        assert assignments_mock == get_assignments.run(pool_mock)
        assert (
            pool_mock.id
            == toloka_client_mock.get_assignments.mock_calls[0][2]["pool_id"]
        )

    def test_get_assignments_by_pool_id(
        self, toloka_client_mock, pool_mock, assignments_mock
    ):
        assert assignments_mock == get_assignments.run(pool_mock.id)

    def test_get_assignments_with_status(
        self, toloka_client_mock, pool_mock, assignments_mock
    ):
        assert assignments_mock == get_assignments.run(pool_mock.id, ["ACCEPTED"])
        assert ["ACCEPTED"] == toloka_client_mock.get_assignments.mock_calls[0][2][
            "status"
        ]


class TestGetAssignmentsDf:
    def test_get_assignments_df_by_pool_object(
        self, toloka_client_mock, pool_mock, assignments_df_mock
    ):
        assert assignments_df_mock.equals(get_assignments_df.run(pool_mock))
        assert (
            pool_mock.id
            == toloka_client_mock.get_assignments_df.mock_calls[0][2]["pool_id"]
        )
    ...
    def test_get_assignments_df_by_pool_id(
        self, toloka_client_mock, pool_mock, assignments_df_mock
    ):
        assert assignments_df_mock.equals(get_assignments_df.run(pool_mock.id))

    def test_get_assignments_df_with_status(
        self, toloka_client_mock, pool_mock, assignments_df_mock
    ):
        assert assignments_df_mock.equals(
            get_assignments_df.run(pool_mock.id, "APPROVED")
        )
        assert ["APPROVED"] == toloka_client_mock.get_assignments_df.mock_calls[0][2][
            "status"
        ]

    def test_get_assignments_df_with_kwargs(
        self, toloka_client_mock, pool_mock, assignments_df_mock
    ):
        kwargs = {
            "status": ["APPROVED"],
        ...
        assert assignments_df_mock.equals(
            get_assignments_df.run(pool_mock.id, **kwargs)
        )
